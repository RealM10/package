...
#include <graphlab/graph/ingress/distributed_package_ingress.hpp>
...
namespace graphlab {
  ...
  public:
    ...
    enum cuts_type {VERTEX_CUTS = 0, EDGE_CUTS, HYBRID_CUTS,PACKAGE_CUTS, HYBRID_GINGER_CUTS,
      NUM_CUTS_TYPES};
...
#pragma omp parallel for
  ...
  private:
    ...
    void set_ingress_method(const std::string& method,
		...
		size_t threshold = 100, size_t theta = 100, size_t nedges = 0, size_t nverts = 0,
        size_t interval = std::numeric_limits<size_t>::max()) {
      ...
      if (method == "oblivious") {
        ...
        if (rpc.procid() == 0) logstream(LOG_EMPH) << "Use package ingress" << std::endl;
        ingress_ptr = new distributed_hybrid_ingress<VertexData, EdgeData>(rpc.dc(), *this, threshold);
        set_cuts_type(PACKAGE_CUTS);
