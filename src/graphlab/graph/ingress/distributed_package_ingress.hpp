...
namespace graphlab {
  ...
  public:
	...
	typedef typename boost::unordered_map<vertex_id_type, vertex_id_type > vertex_hash_table_type;
	vertex_hash_table_type vht;

	typedef typename std::pair<vertex_id_type, vertex_id_type> hash_pair_type;
  ...
  public:
    ...
    void add_edge(vertex_id_type source, vertex_id_type target,
		...
		oht[source].push_back(target);
		iht[target].push_back(source);
		fht[source] = 0;
		fht[target] = 0;
      const edge_buffer_record record(source, target, edata);      
#ifdef _OPENMP
      hybrid_edge_exchange.send(0, record, omp_get_thread_num());
#else
      hybrid_edge_exchange.send(0, record);
#endif
    } // end of add edge
	...
	void  package_construct{
		...
		if (iht[iter->first].size() <= threshold){
		int min_rank = rank[0][0];
		int rank_flag = 0;
		for (int i = 1; i < 10; i++){
		if (min_rank>rank[i][0]){
		min_rank = rank[i][0];
		rank_flag = i;
		}
		}
		if (min_rank < (oht[iter->first].size() + iht[iter->first].size())) {
		rank[rank_flag][0] = (oht[iter->first].size() + iht[iter->first].size());
		rank[rank_flag][1] = iter->first;
		}
		}
		}
		...
		vertex_id_type temp;
		int first = rank[ri][1];
		dht[first] = iht[first].size();
		if (fht[first] == 0){
		if (iht[first].size() <= threshold){
		fht[first] = 1;
		for (size_t ix = 1; ix < nprocs; ix++){
		if (temp2>count[ix]){
		temp2 = count[ix];
		min_proc = ix;
		}
		...
		vht[first] = min_proc;
		for (size_t ix = 1; ix < nprocs; ix++)
		vht_exchange.send(ix, hash_pair_type(first, min_proc));
		for (size_t i = 0; i < iht[first].size(); i++){
		temp = iht[first][i];
		if (fht[temp] == 0){
		if (iht[temp].size() <= threshold)
		count[min_proc] += iht[temp].size() + oht[temp].size();

		fht[temp] = 2;

		vht[temp] = min_proc;
		for (size_t ix = 1; ix < nprocs; ix++)
		vht_exchange.send(ix, hash_pair_type(temp, min_proc));


		}
		}

		for (size_t i = 0; i < oht[first].size(); i++){
		temp = oht[first][i];
		if (fht[temp] == 0){
		if (iht[temp].size() <= threshold)
		count[min_proc] += iht[temp].size() + oht[temp].size();
		...
		*/
		...
		for (size_t ix = 0; ix < nprocs; ix++)
			count[ix] = 0;
		for (target_hash_table_type::iterator iter = iht.begin(); iter != iht.end(); iter++){
			//   hybrid_rpc.cout() << iht[iter->first].size() << std::endl;			  
			dht[iter->first] = iht[iter->first].size();
			vertex_id_type temp;
			vertex_id_type tempi;
			procid_t min_proc = 0;
			int temp2 = count[0];
			if (fht[iter->first] == 0){
				if (iht[iter->first].size() <= threshold&&oht[iter->first].size() <= 10 * threshold){
					fht[iter->first] = 1;
					for (size_t ix = 1; ix < nprocs; ix++){
						if (temp2>count[ix]){
							temp2 = count[ix];
					...
					for (size_t i = 0; i < iht[iter->first].size(); i++){
						temp = iht[iter->first][i];
						if (fht[temp] == 0){
							if (iht[temp].size() <= threshold)
								count[min_proc] += iht[temp].size() + 1;
							if (iht[temp].size() <= threshold&&oht[temp].size() <= threshold){
								for (size_t ii = 0; ii < iht[temp].size(); ii++){
									tempi = iht[temp][ii];
									if (fht[tempi] == 0){
										if (iht[tempi].size() <= threshold)
											count[min_proc] += iht[tempi].size() + 1;
										fht[tempi] = 3;
										vht[tempi] = min_proc;
										for (size_t ix = 1; ix < nprocs; ix++)
											vht_exchange.send(ix, hash_pair_type(tempi, min_proc));
									}
								}
								for (size_t ii = 0; ii < oht[temp].size(); ii++){
									tempi = oht[temp][ii];
									if (fht[tempi] == 0){
										if (iht[tempi].size() <= threshold)
											count[min_proc] += iht[tempi].size() + 1;
					...
					for (size_t i = 0; i < oht[iter->first].size(); i++){
						temp = oht[iter->first][i];
						if (fht[temp] == 0){
							if (iht[temp].size() <= threshold)
								count[min_proc] += iht[temp].size() + 1;
							if (iht[temp].size() <= threshold&&oht[temp].size() <= threshold){
		...
		logstream(LOG_INFO) << " iht end of clear " << std::endl;
		iht.clear();
		oht.clear();
		fht.clear();
		vht_exchange.flush();
		...
		for (size_t i = 0; i < hybrid_edges.size(); i++) {
			procid_t owning_proc = -1;
			edge_buffer_record& rec = hybrid_edges[i];
			if (dht[rec.target] <= threshold){

				if (vht.find(rec.target) != vht.end()){
					owning_proc = vht[rec.target];
					//logstream(LOG_INFO) << "low degree rec.target= " << rec.target << " vht= " << vht[rec.target] << std::endl;
				}
				/*
				else if (vht.find(rec.source) != vht.end()&&vht[rec.source]==rec.source){
				owning_proc = rec.source % nprocs;
				//logstream(LOG_INFO) << "low degree rec.source= " << rec.source << " vht= " << vht[rec.source] << std::endl;
				}
				*/
				else {
					owning_proc = rec.target%nprocs;
					//logstream(LOG_INFO) << "low degree rec.target= " << rec.target << " vht= NULL" << std::endl;
				}
				//logstream(LOG_INFO) << rec.source << " " << fht[rec.source] << " " << rec.target<<" " << fht[rec.target] << " low degree  owning_proc: " << owning_proc << std::endl;
				if (owning_proc != l_procid){
					// re-send the edge of high-degree vertices according to source
					hybrid_edge_exchange.send(owning_proc, rec);
				...
				if (vht.find(rec.source) != vht.end()){
					owning_proc = vht[rec.source];
				}

				/*
				if (vht.find(rec.target) != vht.end()&&vht.find(rec.source)!=vht.end()){
				owning_proc = vht[rec.target] % nprocs;
				//logstream(LOG_INFO) << "high degree rec.target= " << rec.target << " vht= " << vht[rec.target] << std::endl;
				}
				else if (vht.find(rec.source) != vht.end())
				{
				owning_proc = vht[rec.source] % nprocs;
				//logstream(LOG_INFO) << "high degree rec.source= " << rec.source << " vht= " << vht[rec.source] << std::endl;
				}
				*/
				else {
					owning_proc = rec.source %nprocs;
					//logstream(LOG_INFO) << "high degree rec.source= " << rec.source << " vht= NULL" << std::endl;
				}
				// logstream(LOG_INFO) << rec.source << " " << fht[rec.source] << " " << rec.target << " " << fht[rec.target] << " high degree  owning_proc: " << owning_proc << std::endl;
				if (owning_proc != l_procid){
					// re-send the edge of high-degree vertices according to source
					hybrid_edge_exchange.send(owning_proc, rec);
					// set re-sent edges as empty for skipping
        ...
        foreach(const vid2lvid_pair_type& pair, vid2lvid_buffer) {
          ...
          if (standalone)
            vrec.owner = 0;
			  ...
			  else vrec.owner = pair.first % nprocs;
		  }
        }
